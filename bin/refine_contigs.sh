...
# ------------------------------------------------------------------------------------------------ #
# refine_contigs.sh
#     Take reads and map them to contigs for assembly refinement
# ------------------------------------------------------------------------------------------------ #
echo -e "Welcome to 'refine_contigs.sh' !"
...
usage() { echo -e "\nERROR: Missing sample name (needed for naming files) or input files. \n" \
                  "Proper usage for mapping reads to contigs: \n\n" \
                  "$0 -s my_sample -r reads.fastq -c contigs.fasta \n\n" \
                  "Optional parameters: \n" \
                  "-o (output directory for saving refined files; [default = './' (current directory)]) \n" \
                  "-t (number of threads/CPUs); [default = 1] \n\n"
                  "Example of a complex run: \n" \
                  "$0 -s my_sample-r sample-reads.fq -c sample-contigs.fasta -t 4 -o './' \n\n" \
...
cat /dev/null > ${sample}.refine_contigs.log
...
function refinement () {
    ...
    # --FUNCTION---------------------------------------------------------------------------------- #
    # Name:        refinement
    # Description: Map reads to the newly assembled contigs in order to
    ...
    # -------------------------------------------------------------------------------------------- #
    # Log info
    echo "Began mapping at:    $(date)" | \
    tee -a ${sample}.refine_contigs.log
    # -------------------------------------------------------------------------------------------- #
    ...
    # Convert the aligned-reads (BAM) into a pileup (VCF)
    bcftools mpileup \
        --threads $threads \
        -d 1000000 \
        -f $contigs \
